...
#            return True
    ...
    def is_valid_candidate_0(self, path_label):
        is_good = False
        sent=path_label.split(' ')
        list_pos=[sent[i].split('::')[1].lower() for i in range(len(sent)) if len(sent[i].split('::'))>1]
        if len(list_pos)>1 :
            if (list_pos[0]=='num') & (list_pos[1]!='noun'):
                is_good=False
            elif ('adj' not in list_pos):
                is_good=False
            elif ('verb' in list_pos)&(('noun' in list_pos)|('adj' in list_pos)|('adv' in list_pos)):
                list_word=[sent[i].split('::')[0].lower() for i in range(len(sent))]
                list_tag=[sent[i].split('::')[2].lower() for i in range(len(sent)) if len(sent[i].split('::'))>2]
                if any(['peux', 'peut' , 'veux', 'veut']) in list_word:
                    if 'verb__verbform=inf' in list_tag :
                        is_good=True
                else:
                    if 'verb_verbform=fin' in [list_tag[i][:5]+list_tag[i][-12:] for i in range(len(list_tag))]:
                        is_good= True
        else:
            if list_pos[0]=='adj':
                is_good=True                
...
import pandas as pd
import spacy
from opinosis.graph.custom import PosTagGraph

NLP = spacy.load("en_core_web_sm")

#os.chdir('/Users/maryam/Opinosis')
data=pd.read_csv('data/BananaSlicer.csv')   
texts=data['review_text'][:100] 

import time
start=time.time()
#graph = PosTagGraph(texts.drop([377,408,457][:500],axis=0), NLP, node_separator="::")
graph = PosTagGraph(texts, NLP, node_separator="::")

G = graph.create_graph()
creat_graph_time=time.time()-start

start=time.time()
params={'config_max_summaries':7.0,'config_vsn_threshold':30,'config_min_redundancy':1,
        'config_permissable_gap':1,'config_scoring_function':3,'config_attachment_after':2,
        'config_normalize_overallgain':True,'config_turn_on_collapse':True,'config_turn_on_dup_elim':True,
        'p_max_sent_length':18,'p_min_sent_length':2,'config_duplicate_threshold':0.35,'config_duplicate_collapse_threshold':0.4}

opinosis = BasicSummarizerEnglish(G,params)

#opinosis = BasicSummarizerEnglish(G)

#opinosis.set_params(params)
final_sentences_results = opinosis.start()
#print(final_sentences_results)

summarizer_time=time.time()-start